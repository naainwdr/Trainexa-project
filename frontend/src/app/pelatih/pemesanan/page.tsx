    ...
    <div style={{ minHeight: "100vh", background: "#000", color: "#fff", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <main style={{ padding: "120px 72px 80px", maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "48px" }}>
          <h1 style={{ fontSize: "36px", fontWeight: "800", marginBottom: "12px" }}>Data Booking</h1>
          <p style={{ color: "#aaa", fontSize: "16px" }}>Kelola data booking sesi</p>
        </div>

        {/* Booking Cards */}
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          {bookings.map((booking) => (
            <div key={booking.id} style={{ backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "24px", padding: "24px 32px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "24px" }}>
                {/* User Avatar Initials */}
                <div style={{ width: "72px", height: "72px", backgroundColor: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: "16px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "28px", fontWeight: "800", color: "#fff" }}>
                  {booking.initials}
                </div>
                
                {/* Booking Info */}
                <div>
                  <h3 style={{ fontSize: "24px", fontWeight: "800", marginBottom: "8px", display: "flex", alignItems: "center", gap: "8px" }}>
                    {booking.name}
                  </h3>
                  <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#aaa", fontSize: "16px", marginBottom: "12px" }}>
                    <span style={{ fontSize: "14px" }}>{booking.icon}</span> {booking.sport}
                  </div>
                  
                  {/* Row Details */}
                  <div style={{ display: "flex", gap: "20px", color: "#fff", fontSize: "15px", fontWeight: "600" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <span>📅</span> {booking.date}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <span style={{ color: "#ef4444" }}>🕒</span> {booking.time}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <span style={{ color: "#facc15" }}>🔞</span> {booking.type}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <span style={{ color: "#facc15" }}>🪙</span> {booking.price}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <span style={{ color: "#ef4444" }}>📍</span> {booking.location}
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Action Button */}
              <Link href="/pelatih/pemesanan/report">
                <button style={{ backgroundColor: "#fcc419", color: "#1a1a1a", border: "none", padding: "12px 32px", borderRadius: "12px", fontSize: "18px", fontWeight: "800", cursor: "pointer" }}>
                  Report
                </button>
              </Link>
            </div>
